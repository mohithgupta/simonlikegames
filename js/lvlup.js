...
// I gave user_pattern.length==level because without this line you can increase your level each time you hit spcebar, ridiculous rght?
    ...
    alert("Complete this level for the next pattern.");
  }
...
$(".start-btn").click(function(){ // this is to start auto_pattern when mobile users click start
  if(user_pattern.length==level){
    ...
    alert("You must complete the level successfully to go to the next level");
  }
});
...
function start_auto_pattern() {
  ...
  // so you win just after the auto_pattern stops. RIDICULOUS!!!!
...
$(".btn").click(function() { //this is for pushing to user pattern when user clicks the button
  if (auto_pattern.length==level && level!=0){
    // if level!=0 condition is not included , we cannot press a button before we press space bar
    // because at level=0, auto_pattern.length=0 so the user_pattern will start recording and you will lose the game as auto_pattern.length is still =0
    playsound(this.id);
 ...
 { // this setTimeout delays alert msg so that the audio of the btn pressed last is played
  if(full_pattern_check())
  {
    ...
    { // due to this timeout func the audio can be played before the alert msg shows up
      if($(".start-btn").css("display","block")){
        $("h1").html("Congratulations!!! Press the button below to start the next pattern. ");
        $(".start-btn").html("Start the next pattern");
      }
      else{
      $("h1").html("Congratulations!!! You deserve a Level up!!<br>Press Spacebarkey to continue");
    }
     auto_pattern=[]; // refreshes the pattern when the player wins
    },10);
  }
  if(each_step_check())
  {
    ...
    setTimeout(function()
    {
     alert("What did it cost? Everything... Play from level 1 again now!!");
     $("h1").css("font-size","1.2rem");
     $("h1").html("Condemnations!! You don't deserve a Level up!! Refresh the page in 5 seconds or I'll refresh it!!");
     setTimeout(function () {
       $("h1").html("Ok then, I'll refresh it!!");
     }, 5500);
    setTimeout(function(){
      location.reload();
     }, 7500);
     auto_pattern=[]; // refreshes the pattern when the player loses
   }, 300); // due to this timeout func the audio of wrong btn can be played before the alert msg shows up
  }
 }, 10); // this setTimeout delays alert msg so that the audio of the btn pressed last is played
}
